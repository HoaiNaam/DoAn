...
$total_completed_orders = $conn->query("SELECT COUNT(*) FROM `orders` WHERE `payment_status` = 'completed' $search_condition")->fetchColumn();


// Truy vấn đơn hàng với phân trang và tình trạng "completed"
$select_orders = $conn->prepare("SELECT * FROM `orders` WHERE `payment_status` = 'completed' $search_condition LIMIT :offset, :records_per_page");
...
if (isset($_GET['delete'])) {
    ...
    header('location:money_earned.php');
}
...
<section class="placed-orders">
    <h1 class="heading">Doanh thu</h1>
        <section class="summary">
            <p>Tổng số đơn đã hoàn thành: <?= $total_completed_orders; ?></p>
        </section>
    ...
    <p class="pagination" style="font-size: 1.5rem;">Trang:
        ...
        if ($total_pages > 1) {
            for ($i = 1; $i <= $total_pages; $i++) {
                if ($i == $page) {
                    echo '<a href="money_earned.php?page=' . $i . '" class="current-page">' . $i . '</a>';
                } else {
                    echo '<a href="money_earned.php?page=' . $i . '">' . $i . '</a>';
                }
            }
        }
        ?>
